...
DROP TABLE IF EXISTS [cdw_cache_staging].[{project_schema}].[lab_epic];
CREATE TABLE [cdw_cache_staging].[{project_schema}].[lab_epic] (
    lab_component_result_key    int             not null primary key,
...
WHERE
    lr.result_datetime between @date_start and @date_stop
    and lr.is_final = 1
