...
DROP TABLE IF EXISTS [cdw_cache_staging].[{project_schema}].[birth_epic];
CREATE TABLE [cdw_cache_staging].[{project_schema}].[birth_epic] (
    birth_epic_index                    int             identity(1,1) primary key,
    mrn_mpi_mother                      int             not null,
    baby_mrn_epic_durable               int             not null,
    birth_index_within_mother           smallint        null,   -- row_number per mother
    birth_datetime                      datetime        null,
    -- Pregnancy timeline:
    pregnancy_estimated_start_date      date            null,
    pregnancy_estimated_end_date        date            null,
    pregravid_weight                    numeric(18,1)   null,
    pregravid_bmi                       numeric(18,1)   null,
    labor_start_datetime                datetime        null,
    mother_age_years                    int             null,
    -- Delivery:
    delivery_method                     varchar(80)     null,
    labor_type                          varchar(80)     null,
    presentation_type                   varchar(80)     null,
    placenta_method                     varchar(50)     null,
    antenatal_steroids                  varchar(80)     null,
    -- Gestational age:
    gestational_age_days                int             null,
    gestational_age_weeks               tinyint         null,
    gestational_age_days_remainder      tinyint         null,
    -- Multiple births:
    multiple_delivery_count             tinyint         null,
    multiple_delivery_order             tinyint         null,
    -- Infant measurements:
    birth_weight_grams                  numeric(10,3)   null,
    birth_length                        numeric(8,3)    null,
    head_circumference                  numeric(8,3)    null,
    baby_discharge_weight               numeric(10,3)   null,
    baby_inpatient_length_of_stay_days  smallint        null,
    -- APGAR scores:
    total_apgar_one_minute              tinyint         null,
    total_apgar_five_minute             tinyint         null,
    total_apgar_ten_minute              tinyint         null,
    -- Flags:
    breast_milk_given                   bit             null,
    spontaneous_vaginal_delivery        bit             null,
    labor_attempted                     bit             null,
);

INSERT INTO {project_schema}.birth_epic
SELECT distinct
...
WHERE b.birth_datetime between @date_start and @date_stop
