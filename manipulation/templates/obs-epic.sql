...
CREATE TABLE #pt_epic (
    mrn_epic_durable    int     not null primary key,
...
DROP TABLE IF EXISTS [cdw_cache_staging].[{project_schema}].[obs_epic];
CREATE TABLE [cdw_cache_staging].[{project_schema}].[obs_epic] (
    obs_epic_index              int             identity(1,1) primary key,
    mrn_mpi                     int             not null,
...
--   and cast(e.encounter_start_date as date) between @date_start and @date_stop
