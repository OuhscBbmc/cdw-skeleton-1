...
declare @date_start   date = '2023-06-03';   -- Epic go-live floor
declare @date_stop    date = '{date_stop}';


DROP TABLE IF EXISTS [cdw_cache_staging].[{project_schema}].[medication_epic];
CREATE TABLE [cdw_cache_staging].[{project_schema}].[medication_epic] (
    med_epic_index              int             identity(1,1) primary key,
    ...
    simple_generic_name         varchar(300)    null,
    pharmaceutical_class        varchar(300)    null,
    pharmaceutical_subclass     varchar(300)    null,
    therapeutic_class           varchar(300)    null,
    start_datetime              datetime2       not null,
    administered                int             not null,
    route                       varchar(300)    null,
    minimum_dose                numeric(18,2)   null,
    maximum_dose                numeric(18,2)   null,
    dose_unit                   varchar(300)    null,
    -- Study classification:
    med_category                varchar(100)    null,
);

INSERT INTO {project_schema}.medication_epic
SELECT
    na.mrn_mpi
    ,na.mrn_epic_durable
...
WHERE
    m.start_date between @date_start and @date_stop
