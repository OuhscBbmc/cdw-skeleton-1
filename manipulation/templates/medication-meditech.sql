...
DROP TABLE IF EXISTS [cdw_cache_staging].[{project_schema}].[medication_meditech];
CREATE TABLE [cdw_cache_staging].[{project_schema}].[medication_meditech] (
    med_meditech_index      int             identity(1,1) primary key,
...
INSERT INTO {project_schema}.medication_meditech
SELECT
    m.account_number
    ,na.mrn_mpi
    ,m.mrn_meditech_internal
    ,m.source_meditech
    ,m.admin_datetime
    ,m.medication_name
    ,m.dose_unit
...
WHERE
    m.admin_datetime between @date_start and @date_stop
