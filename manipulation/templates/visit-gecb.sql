...
DROP TABLE IF EXISTS [cdw_cache_staging].[{project_schema}].[visit_gecb];
CREATE TABLE [cdw_cache_staging].[{project_schema}].[visit_gecb] (
    visit_gecb_index            int             identity(1,1) primary key,
    visit_number                int             not null unique,
    mrn_mpi                     int             not null,
    mrn_gecb                    int             not null,
    visit_index_within_patient  bigint          null,
    ...
    billing_loc_name            varchar(100)    null,
    visit_status                varchar(30)     not null,
);

INSERT INTO {project_schema}.visit_gecb
SELECT
    fs.visit_number
    ,na.mrn_mpi
    ,fs.mrn_gecb
...
WHERE
    fs.appt_date between @date_start and @date_stop
